...
function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    if (items.length === 0 && !successOrderCode && step !== 3) {
      console.log('[DEBUG] CheckoutForm: Redirecting to cart (empty)');
      router.push('/cart');
    }
  ...
  const totalWithVat = subtotal + (subtotal * vatRate);
      ...
      {step === 1 && (
        <>
          ...
          <div style={{ padding: '0 20px 12px' }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: COLORS.smoke, textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Full Name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" style={{
              width: '100%', background: COLORS.sand, border: 'none', borderRadius: 14, padding: '12px 14px', fontSize: 14, color: COLORS.bark, outline: 'none'
            }} />
          </div>
          <div style={{ padding: '0 20px 12px' }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: COLORS.smoke, textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Phone Number</label>
            <div style={{ display: 'flex', gap: 10 }}>
              <select value={countryCode} onChange={(e) => setCountryCode(e.target.value)} style={{ width: 100, background: COLORS.sand, border: 'none', borderRadius: 14, padding: '12px 10px', fontSize: 14, color: COLORS.bark, outline: 'none' }}>
                {countryCodes.map(cc => <option key={cc.code} value={cc.code}>{cc.flag} +{cc.code}</option>)}
              </select>
              <input value={phone} onChange={(e) => setPhone(e.target.value)} type="tel" placeholder="51000000" style={{ flex: 1, background: COLORS.sand, border: 'none', borderRadius: 14, padding: '12px 14px', fontSize: 14, color: COLORS.bark, outline: 'none' }} />
            </div>
          </div>
          <div style={{ padding: '0 20px 12px' }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: COLORS.smoke, textTransform: 'uppercase', marginBottom: 6, display: 'block' }}>Email</label>
            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="email@example.com" style={{ width: '100%', background: COLORS.sand, border: 'none', borderRadius: 14, padding: '12px 14px', fontSize: 14, color: COLORS.bark, outline: 'none' }} />
          </div>
        ...
        <>
          ...
          {['cod', 'card', 'apple'].map(id => (
            <div key={id} onClick={() => setPayMethod(id)} style={{
              ...
              <div style={{ width: 40, height: 28, borderRadius: 6, background: COLORS.sand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                {id === 'cod' ? '💵' : id === 'card' ? '💳' : '🍎'}
              </div>
              <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.bark }}>
                {id === 'cod' ? 'Cash on Delivery' : id === 'card' ? 'Card' : 'Apple Pay'}
              </div>
              <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${payMethod === id ? COLORS.terracotta : COLORS.smoke}`, marginLeft: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {payMethod === id && <div style={{ width: 8, height: 8, borderRadius: '50%', background: COLORS.terracotta }} />}
              </div>
            </div>
          ))}

          {error && <div style={{ margin: '0 20px 12px', padding: 12, borderRadius: 12, background: '#FEF2F2', border: '2px solid #FECACA', color: '#DC2626', fontSize: 14, fontWeight: 600 }}>{error}</div>}
