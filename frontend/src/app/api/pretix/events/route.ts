...
const ORGANIZERS = [
  ...
  { slug: 'yadawi-sa', token: process.env.PRETIX_API_TOKEN_SA || 'SA_3ll9f5237hcv96ioakrebef35qvl7qvuurfp3ih46oldfc5i9abmrkdceirozhsz' },
];
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const orgPromises = ORGANIZERS.map(async (org) => {
      try {
        const response = await fetch(`${PRETIX_API_URL}/api/v1/organizers/${org.slug}/events/`, {
        ...
        if (response.ok) {
          ...
          const enrichedEvents = await Promise.all(events.map(async (e: any) => {
            ...
            try {
              ...
              console.error(`Error fetching items for ${e.slug}:`, err);
            }
